...
- Le statut de l’exécution (succès ou échec)
...
Afin de visualiser le tout, on pourrait générer des dashboards de monitoring afin d’observer visuellement toutes les métriques disponibles et valider la santé du système.
...
Si le résultat des métriques est satisfaisant, on sauvegarde le nouveau modèle en format .pkl par exemple, puis on le déploie afin qu’il soit utilisé pour les prochains calculs de recommandations. Comme pour le calcul des recommandations, ce processus pourrait être orchestré à l’aide d’un outil comme Apache Airflow.
